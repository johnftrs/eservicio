<div class="card">
				...
				<tbody>
					...
					<tr>
						...
						<td class="centrado">
							...
							<div style="font-size: 13px; height: 12px; font-weight: bold;">( {{$account->dispenser->nombre}} )</div>
							@endforeach
						</td>
						@if (Auth::user()->role_id <= 3)
						...
						@endif
						<td class="centrado">
							<button class="btn btn-min success limpiar_iframe" wire:click="agregar_ticket({{ $report->id }})"><i class="mdi mdi-ticket-confirmation"></i>Vales</button>
						</td>
						<td class="centrado">
						...
						<td class="centrado">
							@if ($report->editar())
							@if (count(\App\Models\Conciliation::where('fecha',$report->fecha)->get())==0)
	...
	<div class="modal-dialog panel visible">
		...
		<div class="panel-body" >
			...
			@elseif($modal['accion']=='ticket')
			@include('admin.report.forms.formActivar')
			@else
		...
		<div class="panel-footer col-4 default-soft">
			...
			@elseif($modal['accion']=='ticket')
			<button wire:click="save_ticket()" type="submit" class="btn btn-min warning col-1">Guardar</button>
			<div class="nota_footer">Efectivo Total: Bs. {{number_format($suma, 2, ',', '.')}}</div>
