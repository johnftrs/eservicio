<div class="card">
	...
	foreach (Auth::user()->role->functionalities as $func) {
		if ($func->code=='ECON'){ $editar=true; }
		if ($func->code=='DCON'){ $eliminar=true; }
	}
	?>
	<div class="card-header primary-low">
		<h5 class="card-title"><i class="mdi mdi-briefcase-check"></i>Conciliación de Arqueo</h5>
	...
	<div class="card-body">
		<div class="table-responsive">
			<table class="table table-hover">
				<thead>
					<th class="centrado">Conciliado</th>
					<th class="centrado">Depósito</th>
				...
				<tbody>
					...
					<tr>
						<td class="centrado">{{\Carbon\Carbon::parse($conciliation->fecha)->format('d/m/y')}}</td>
						<td class="centrado">{{\Carbon\Carbon::parse($conciliation->fecha_deposito)->format('d/m/y')}}</td>
						<td class="centrado">{{number_format($conciliation->monto, 2, ',', '.')}}</td>
						<td class="derecha">{{$conciliation->observaciones}}</td>
						<td class="centrado">
							<button class="btn btn-min info" wire:click="openModalPDF({{ $conciliation->id }})"><i class="mdi mdi-printer"></i>PDF</button>
						</td>
						@if ($editar)
						<td class="centrado">
							<button class="btn btn-min warning" wire:click="edit({{$conciliation->id}})"><i class="mdi mdi-pencil"></i>Editar</button>
						</td>
						@endif
						@if ($eliminar)
						<td class="centrado">
							<button type="button" wire:click="select({{$conciliation->id}})" class="btn btn-min danger"><i class="mdi mdi-trash-can-outline"></i>Borrar</button>
						</td>
						@endif
					</tr>
					@endforeach
	...
	<div class="modal-dialog panel primary visible">
		...
		<div class="panel-body" >
			...
			@else
			@include('admin.conciliation.forms.form')
			@endif
		</div>
