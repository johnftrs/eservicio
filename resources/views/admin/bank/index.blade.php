<div class="card">
	...
	<div class="card-header primary-low">
		<h5 class="card-title"><i class="mdi mdi-bank"></i>Bancos</h5>
			...
			<table class="table table-hover">
				...
				<tbody>
					...
					<tr>
						<td>{{$bank->nombre}}</td>
						<td>{{$bank->cuenta}}</td>
						<td class="centrado">{{$bank->moneda}}</td>
						<td>{{number_format($bank->monto, 2, ',', '.')}}</td>
						@if ($editar)
						<td class="centrado">
							<button class="btn btn-min warning" wire:click="edit({{$bank->id}})"><i class="mdi mdi-pencil"></i>Editar</button>
						</td>
						@endif
						@if ($eliminar)
						<td class="centrado">
							<button type="button" wire:click="select({{$bank->id}})" class="btn btn-min danger"><i class="mdi mdi-trash-can-outline"></i>Borrar</button>
						</td>
						@endif
					</tr>
					@endforeach
	...
	<div class="modal-dialog panel primary visible">
		<div class="panel-heading">
			<h4 class="panel-title"><b style="color: white;">Registro de Bancos</b></h4>
			<a class="btn-close btn danger" wire:click="limpiar">&times;</a>
		</div>
		<div class="panel-body" >
			@include('admin.bank.forms.form')
		</div>
